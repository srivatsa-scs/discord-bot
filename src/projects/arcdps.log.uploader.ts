...
import { arcDpsLogLogger, logger } from "../logger/log4js.adapter";
...
export function uploaderFunction(client: any) {
  ...
  watcher.on("add", async (path: any) => {
    ...
    try {
      const resp: any = await axios.post(uploadUrl, form, {
        headers: form.getHeaders(),
      });
      const metaData: any = await axios.get(
        `${metaDataUrl}${resp.data.permalink.substring(19)}`
      );
      const fightname = `${metaData.data.fightName}${
        resp.data.encounter.isCM ? " CM" : ""
      }`;
      ...
      const embed = new MessageEmbed()
        ...
        .setThumbnail(
          metaData.data.fightIcon ||
            boss.get(resp.data.encounter.bossId).thumbnail
        )
        .addFields(
          {
            name: "Log Uploaded By",
            value: `${metaData.data.recordedBy || "Unknown"}`,
          },
          { name: "Result", value: resp.data.encounter.success ? "✅" : "⛔" },
          {
            ...
            value: `${
              ...
              timeFormatter(resp.data.encounter.duration)
            }`,
          }
        )
        .setFooter(`ArcDps Version: ${resp.data.evtc.version}`)
        .setTimestamp();
      arcDpsLogLogger.info(
        `Log was successfully uploaded for Boss: ${fightname} by user ${
          metaData.data.recordedBy || "Unknown"
        }`
      );
      ...
      if (
        ...
        err.response.data.error ===
          "Encounter is too short for a useful report to be made"
      ) {
        arcDpsLogLogger.error(
          `Log was not uploaded because ${err.response.data.error}`
        );
        return;
