    ...
    try {
      ...
      const embed = new MessageEmbed()
        ...
        .addFields(
          ...
          {
            name: 'Duration',
            value: `${metaData.data.duration || timeFormatter(resp.data.encounter.duration)}`,
          }
        )
        .setFooter(`ArcDps Version: ${resp.data.evtc.version}`)
        .setTimestamp();
      arcDpsLogLogger.info(
        `Log was successfully uploaded for Boss: ${fightname} by user ${metaData.data.recordedBy || 'Unknown'}`
      );
      client.channels.cache.get(logChannelId).send(embed);
    } catch (err: any) {
      if (
        err.response.status === 403 &&
        err.response.data.error === 'Encounter is too short for a useful report to be made'
      ) {
